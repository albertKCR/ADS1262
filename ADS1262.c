...
uint32_t ADS1262_ReadData()
{
    ...
    sprintf(charData, "%i, ", rxData[0]);
    CDC_Transmit_FS((uint8_t *)charData, strlen(charData));
    sprintf(charData, "%i ,", rxData[1]);
    CDC_Transmit_FS((uint8_t *)charData, strlen(charData));
    sprintf(charData, "%i ,", rxData[2]);
    CDC_Transmit_FS((uint8_t *)charData, strlen(charData));
    sprintf(charData, "%i ,", rxData[3]);
    CDC_Transmit_FS((uint8_t *)charData, strlen(charData));
    sprintf(charData, "%i ,", rxData[4]);
    CDC_Transmit_FS((uint8_t *)charData, strlen(charData));
    sprintf(charData, "%i ,", rxData[5]);
    CDC_Transmit_FS((uint8_t *)charData, strlen(charData));
...
void ADS1262_Init()
{
    ...
    ADS1262_WriteRegister(POWER, 0x01); // internal reference enable
    HAL_Delay(10);
    ADS1262_WriteRegister(INTERFACE, 0x05); // status byte during data read, enable checksum during data read
    HAL_Delay(10);
    ADS1262_WriteRegister(MODE0, 0x00); // continuous conversion
    HAL_Delay(10);
    ADS1262_WriteRegister(MODE1, 0x80); // FIR digital filter
    HAL_Delay(10);
    ADS1262_WriteRegister(MODE2, 0x04); // PGA enable with 1 V/V, 20 SPS
    HAL_Delay(10);
    ADS1262_WriteRegister(INPMUX, 0x01); // Positive input AIN0, Negative input AIN1
    HAL_Delay(10);
}
...
{
    ADS1262_WriteRegister(INPMUX, (AINP << 8) | AINN);
}